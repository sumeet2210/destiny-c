...
import { getMyRsvpIds, getSavedIds } from '@/lib/api/social';
...
function SavedContent() {
  const { data, loading, error, reload } = useApi(async () => {
    ...
    return { saved, interestedEvents, savedWithOffers };
  }, []);
      ...
      <header className="relative overflow-hidden rounded-[1.6rem] border border-white/10 bg-[radial-gradient(circle_at_82%_12%,rgba(167,139,250,0.2),transparent_34%),radial-gradient(circle_at_12%_100%,rgba(71,215,255,0.08),transparent_32%),linear-gradient(145deg,#202020,#0d0d0d)] px-5 pt-16 pb-5 shadow-[0_22px_60px_rgba(0,0,0,0.34)] sm:p-8">
        ...
        {data && (
          <div className="relative mt-7 grid grid-cols-3 gap-2 sm:max-w-xl sm:gap-3">
            <SavedStat label="Places" value={data.saved.length} />
            <SavedStat label="Live offers" value={data.savedWithOffers.length} />
            <SavedStat label="Events" value={data.interestedEvents.length} />
          </div>
        )}
      </header>

      {loading && !data ? (
        <LoadingBlock label="Loading your saved scene…" />
      ) : error ? (
        ...
        <>
          <section className="space-y-4" aria-labelledby="saved-places-title">
            ...
            {data.saved.length === 0 ? (
              <Card className="text-text-muted text-center text-sm">
                Nothing saved yet — tap the bookmark on any place you want to
                come back to.{' '}
              ...
              <RestaurantGrid restaurants={data.saved} source="direct" />
            )}
          </section>
          ...
          <section className="space-y-4" aria-labelledby="saved-offers-title">
            ...
            {data.savedWithOffers.length ? (
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {data.savedWithOffers.map((restaurant) => (
                  <Link
                    key={restaurant.id}
              ...
              <Card className="text-text-muted text-center text-sm">
                No live offers at your saved restaurants right now. New drops
          ...
          <section className="space-y-4" aria-labelledby="saved-events-title">
            ...
            {data.interestedEvents.length ? (
              <div className="grid gap-3 md:grid-cols-2">
                {data.interestedEvents.map((event) => (
