// Seed data — the single source of demo content, ported from the Next app's
// lib/data/seed.ts. Powers seed mode: the view layer serves these objects when
// Supabase env vars are absent, so the whole read surface works with no project.
//
// Offer expiries and event times are computed relative to load time so the demo
// always has live offers and upcoming events.
...
function daysFromNow(d, hour = 19) {
  const t = now();
...
const oid = (n) => `00000000-0000-4000-8001-00000000000${n}`;
export const SEED_OWNER_IDS = [1, 2, 3, 4, 5, 6].map(oid);

// Deterministic ids for children so seed-mode links are stable.
let miCounter = 0;
const mid = () =>
...
const photos = (restaurantId, urls, kind = 'gallery') =>
  urls.map((url, i) => ({
    id: phid(),
    restaurant_id: restaurantId,
    url,
...
// reference bookings that exist only in seed.sql, not in seed mode — the
...
const sid = (n) => `00000000-0000-4000-8008-00000000000${n}`;
